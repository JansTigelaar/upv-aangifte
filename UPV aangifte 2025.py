...
# filtered_inbound_material_weight_PL = inbound_material_weight.where(
#     F.col("business_model_desc") == "Private label"
# )

# display(filtered_inbound_material_weight_PL)
# display(filtered_inbound_material_weight_PL.count())

# # Calculate and display the total net_despatch_qty
# total_net_despatch_qty = filtered_inbound_material_weight_PL.agg(
#     F.sum("net_despatch_qty").alias("total_net_despatch_qty")
# ).collect()[0][0]

# print(f"Total net_despatch_qty: {total_net_despatch_qty}")

# # Group by mcc_1_name and aggregate total net despatch quantity
# mcc1_totals = filtered_inbound_material_weight_PL.groupBy("mcc_1_name").agg(
#     F.sum("net_despatch_qty").alias("total_net_despatch_qty")
# )

# display(mcc1_totals)

# filtered_material = filtered_inbound_material_weight_PL.filter(
#     col("Y02 - materiaal").rlike("(?i)katoen|cotton")
# )

# display(filtered_material)
# display(filtered_material.count())
...
# percentage_pattern = r"(\d+)%\s*(?:katoen|cotton)|(?:katoen|cotton)\s*(\d+)%"

# filtered_inbound_material_weight_PL = filtered_inbound_material_weight_PL.withColumn(
#     "cotton_percentage_raw",
#     when(
#         col("Y02 - materiaal").rlike("(?i)katoen|cotton"),
#         when(
#             regexp_extract(col("Y02 - materiaal"), percentage_pattern, 1) != "",
#             regexp_extract(col("Y02 - materiaal"), percentage_pattern, 1)
#         ).otherwise(
#             regexp_extract(col("Y02 - materiaal"), percentage_pattern, 2)
#         )
#     ).otherwise(None)  
# )
...
#     "cotton_percentage",
#     when(
#         (col("cotton_percentage_raw").isNull()) | (col("cotton_percentage_raw") == ""),
#         0
#     ).otherwise(col("cotton_percentage_raw").cast("float"))  # Safely cast to float
# )

# filtered_inbound_material_weight_PL = filtered_inbound_material_weight_PL.withColumn(
#     "cotton_percentage",
#     when(col("cotton_percentage") == 0, 100).otherwise(col("cotton_percentage"))
# )

# filtered_inbound_material_weight_PL = filtered_inbound_material_weight_PL.withColumn(
#     "cotton_weight",
#     (col("cotton_percentage") / 100) * (col("article_gross_weight_num")*0.92) * col("net_despatch_qty")
# )

# filtered_inbound_material_weight_PL = filtered_inbound_material_weight_PL.filter(
#     col("Y02 - materiaal").rlike("(?i)katoen|cotton")
# )

# display(filtered_inbound_material_weight_PL.select(
#     "article_id",
#     "article_desc",
#     "mcc_1_name",
#     "mcc_2_name",
...
# # Group by mcc_1_name and mcc_3_name, and aggregate total net purchase quantity and total weight
# mcc_totals = filtered_inbound_material_weight_PL.groupBy("mcc_1_name", "mcc_3_name").agg(
#     F.sum("net_despatch_qty").alias("total_net_purchase_qty"),
#     F.sum("cotton_weight").alias("total_cotton_weight")
# )

# # Display the result for each mcc_1_name and mcc_3_name category
# display(mcc_totals)
...
# total_cotton_weight = filtered_inbound_material_weight_PL.select(
#     (_sum(col("cotton_weight")) / 1000000).alias("total_cotton_weight_mt")
# )

# total_purchase_qty = filtered_inbound_material_weight_PL.select(
#     (_sum(col("net_despatch_qty"))))
    

# display(total_cotton_weight)
# display(total_purchase_qty)
